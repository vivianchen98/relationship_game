...
class Game1:
    def __init__(self, num_player, scores, actions):
        self.size = num_player
        self.actions = actions
        self.scores = np.array(scores).reshape(self.size, self.size, self.size)
    ...
    def prettyPrint(self):
        # df = pd.DataFrame(self.scores, index=self.actions, columns=self.actions)
        print('------------- Game matrix --------------')
...
class Game:
    ...
    def prettyReturn(self):
        ...
        # game = np.zeros((self.size, self.size))
        for i in range(self.size):
